...
exports.login = (Model) => catchAsync(async (req, res, next) => {
  ...
  const user = await Model.findOne({$or: [{email: email}, {username: email}]}).select('+password');
...
exports.forgotPassword = (Model, type) => catchAsync(async (req, res, next) => {
  // Get user based on email or username
  const user = await Model.findOne({$or: [{email: req.body.email}, {username: req.body.email}]});
  if(!user) return next(new AppError('Account with this email or username not found', 401));
  // Generate the random reset token
  const resetToken = user.createPasswordResetToken();
  await user.save({ validateBeforeSave: false });
  const resetUrl = `http://${req.get('host')}/api/${type}/resetpassword/${resetToken}`;
...
exports.deactivateMe = (Model) => catchAsync(async (req, res, next) => {
  const user = await Model.findByIdAndUpdate(req.user.id, {active: false}).select('+active');
  res.status(200).json({
    success: true,
    message: 'Account has been successfully deactivated! You can activate your account by logging in again',
    data: user
  });
});
